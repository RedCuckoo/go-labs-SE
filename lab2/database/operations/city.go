...
func (c *cityDB) Insert(city database.City) (database.City, error) {
	_, err := c.db.Query("INSERT INTO city (country, name, population, capital) values ($1, $2, $3, $4) returning *", city.Country, city.Name, city.Population, city.Capital)
...
func (c *cityDB) Update(city database.City) (database.City, error) {
	_, err := c.db.Query("UPDATE city SET country=$2, name=$3, population=$4, capital=$5 WHERE id=$1", city.Id, city.Country,city.Name, city.Population, city.Capital)
